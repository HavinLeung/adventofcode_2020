...
def part1(seats):
    ...
    def seat_is_occupied(seats, i, j):
        if 0 <= i < len(seats):
            if 0 <= j < len(seats[i]):
                return seats[i][j] == "#"
        return False

    def num_occupied_adjacent_seats(seats, i, j):
        neighbors = [
            (+0, -1),
            (+0, +1),
            (-1, +0),
        ...
        neighbors = lmap(lambda x: (x[0] + i, x[1] + j), neighbors)
        return count(lambda x: seat_is_occupied(seats, *x), neighbors)
    ...
    while True:
        ...
        for i in range(len(new_seats)):
            for j in range(len(new_seats[i])):
                if new_seats[i][j] == ".":
                    pass
                elif new_seats[i][j] == "L":
                    if num_occupied_adjacent_seats(seats, i, j) == 0:
                        new_seats[i][j] = "#"
                        any_changes = True
                else:
                    if num_occupied_adjacent_seats(seats, i, j) >= 4:
                        new_seats[i][j] = "L"
                        any_changes = True
        seats = new_seats
        if not any_changes:
            break
    return sum(map(lambda row: count(lambda c: c == "#", row), seats))


def part2(seats):
    seats = [list(row) for row in seats]

    def seat_can_see_occupied(seats, x, y, dx, dy):
        while True:
            x += dx
            y += dy
    ...
    def num_occupied_adjacent_seats(seats, i, j):
        ...
        return count(lambda x: seat_can_see_occupied(seats, i, j, *x), dir)
        ...
        for i in range(len(new_seats)):
            for j in range(len(new_seats[i])):
                if new_seats[i][j] == ".":
                    ...
                    if num_occupied_adjacent_seats(seats, i, j) >= 5:
                        new_seats[i][j] = "L"
                        any_changes = True
        # seats = new_seats
        # for row in seats:
